...
echo "============================================"
echo " Templar Kernel GKI 5.10 - Redmi Note 13 Pro"
echo "============================================"

ROOT_DIR="/home/sarkot/templar-kernel-public"
OUTPUT_DIR="/home/sarkot/kernel-output"
MAX_JOBS=2
...
./scripts/kconfig/merge_config.sh -m -O . \
    ...
    arch/arm64/configs/garnet_optimized.fragment 2>&1
...
properties() { '
kernel.string=Templar Kernel GKI 5.10 - Redmi Note 13 Pro 5G (garnet)
do.devicecheck=1
do.modules=0
do.systemless=1
do.cleanup=1
...
AK3
    ...
    cd "$AK3_DIR"
    rm -f "$OUTPUT_DIR/TemplarKernel-GKI5.10-garnet.zip"
    zip -r9 "$OUTPUT_DIR/TemplarKernel-GKI5.10-garnet.zip" * \
        -x ".git/*" "README.md" "LICENSE" 2>&1
    echo "Zip: $OUTPUT_DIR/TemplarKernel-GKI5.10-garnet.zip"
fi
